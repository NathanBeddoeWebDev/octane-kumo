...
import { CaretRight, X, type Icon } from "@octanejs/phosphor-icons";
import {
  Children,
  cloneElement,
  createContext,
...
type NativeDivStyle = Exclude<
  JSX.IntrinsicElements["div"]["style"],
  string | undefined
>;
type SidebarCSSProperties = NativeDivStyle & Record<`--${string}`, string>;

/** Assign an Octane ref (callback, object, or array thereof) imperatively. */
function assignRefValue(ref: unknown, node: unknown): void {
  if (!ref) return;
  if (Array.isArray(ref)) {
    for (const item of ref) assignRefValue(item, node);
    return;
  }
  if (typeof ref === "function") {
    (ref as (node: unknown) => void)(node);
    return;
  }
...
export type SidebarRootProps = JSX.IntrinsicElements["aside"] & {
...
function SidebarRoot({
  ...
  // (Kept as an imperative update in the Octane port for the same reason.)
    ...
    if (openMobile) {
      ...
      requestAnimationFrame(() => {
        const firstFocusable =
          mobileNodeRef.current?.querySelector<HTMLElement>(FOCUSABLE_SELECTOR);
 ...
 *
 * DIVERGENCE: the React implementation wraps Base UI's ScrollArea
 * (`Root`/`Viewport`/`Content`/`Scrollbar`/`Thumb`) for a custom scrollbar and
 * scroll-fade mask. `@octanejs/base-ui@0.1.48` has no scroll-area module, so
 * the Octane port renders a plain `overflow-y-auto` viewport instead. The
 * `data-sidebar="content"` / `data-sidebar="viewport"` attributes (used by
 * `scrollToItem` via `.closest('[data-sidebar="viewport"]')`) and layout
 * classes are preserved; the custom thumb and mask-fade classes are dropped.
 *
 * @example
 * ```tsx
 * <Sidebar.Content>
...
/**
 * Icon for `Sidebar.MenuButton`: a component type (e.g. a Phosphor `Icon`)
 * or an already-created element. (React types this as
 * `ComponentType | ReactNode`; `OctaneNode` is `unknown`, so the element arm
 * is spelled as `ElementDescriptor` to keep the union meaningful.)
 */
export type SidebarMenuButtonIcon =
  | ((props: { className?: string }) => OctaneNode)
  | ElementDescriptor;
...
function SidebarMenuButton({
  ...
  // Render icon — supports both component types and Octane elements.
  // Phosphor `Icon` components satisfy the component arm of the union.
  const iconNode = ((): OctaneNode => {
    if (!IconProp) return null;
    if (isValidElement(IconProp)) return IconProp;
    const Comp = IconProp as (props: { className?: string }) => OctaneNode;
    return createElement(Comp as Icon, {
      className: cn(
        "shrink-0 opacity-40",
  ...
  // structure stays stable across expand/collapse — preventing remounts of
  // the button (which would kill CSS transitions).
...
export type SidebarMenuSubButtonProps = JSX.IntrinsicElements["button"] & {
...
function SidebarMenuSubButton({
  ...
  if (href) {
    button = (
      <LinkComponent
        ref={ref as JSX.IntrinsicElements["a"]["ref"]}
        {...(props as JSX.IntrinsicElements["a"])}
        className={cn(buttonClasses, "no-underline!")}
        href={href}
        to={href}
...
function SidebarClose({
  ...
  return (
    <Button
      ...
      style={style as Exclude<typeof style, string | undefined>}
      className={cn("shrink-0", className)}
      onClick={(e) => {
        onClick?.(e);
        setOpenMobile(false);
...
export type SidebarCollapsibleProps = JSX.IntrinsicElements["div"] & {
 ...
 *
 * Custom implementation (no Base UI dependency) — identical approach in React
 * and Octane, so no fallback is needed here. Keyboard behaviour: when a child
 * receives keyboard focus (`focus-visible`) while collapsed, the section
 * auto-expands. It collapses again on blur unless it was explicitly opened
 * via click or a child has `data-active`.
 *
 * @example
 * ```tsx
 * <Sidebar.MenuItem>
...
// NOTE: `SidebarCollapsible` shares its name with the `SidebarCollapsible`
// union type, which defeats TS expando-function `displayName` assignment, so
// it is attached through a structural cast instead.
(SidebarCollapsible as unknown as { displayName?: string }).displayName =
  "Sidebar.Collapsible";
  ...
  return cloneElement(render, {
    ...
    onClick: (e: MouseEvent) => {
      ...
      const existingOnClick = (
        render.props as { onClick?: (e: MouseEvent) => void }
      ).onClick;
      existingOnClick?.(e);
      toggle();
    },
...
function SidebarCollapsibleContent({
  ...
  // Imperatively set inert — kept imperative in the Octane port, matching
  // the React implementation's approach.
...
export type SidebarSlidingViewsProps = JSX.IntrinsicElements["div"] & {
...
export type SidebarSlidingViewProps = JSX.IntrinsicElements["div"] & {
 ...
 */
function SidebarSlidingView({
  value,
  className,
  children,
  ref,
  ...props
}: SidebarSlidingViewProps) {
