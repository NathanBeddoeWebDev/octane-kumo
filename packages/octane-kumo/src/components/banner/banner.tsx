...
import { isValidElement, type OctaneNode } from "octane";
...
export interface BannerProps extends Omit<
  ...
  "children" | "title"
> {
  /** Icon element rendered before the banner content (e.g. from `@octanejs/phosphor-icons`). */
  icon?: OctaneNode;
        ...
        >
          ...
          >
            {isCompact ? (
              ...
              >
                {title ? (
                  <span className={cn("leading-snug font-medium")}>
                    {title}
                    {!description && hasInlineLinkAction ? (
                      <span
                        className={cn(
                          "ml-1.5 [&_[data-kumo-component=Link]]:inline",
                        )}
                      >
                        {action}
                      </span>
              ...
              <div className={cn("flex flex-col gap-0.5")}>
                {title ? (
                  <p className={cn("leading-snug font-medium")}>{title}</p>
                ) : null}
                {description ? (
